""" REST controller for remote relays management ressource """
import logging
...
from server.rest_api.electrical_panel_controler.rest_model import SingleRelayStatusSchema, RelaysStatusResponseSchema, RelaysStatusQuerySchema
...
bp = Blueprint("remote electrical panel", __name__, url_prefix="/remote/electrical_panel")
""" The api blueprint. Should be registered in app main api object """


@bp.route("/")
class RemoteRelaysStatusApi(MethodView):
    """API to remote retrieve or set electrical panel status"""
    ...
    def get(self):
        ...
        logger.info(f"GET electrical_panel/  (REMOTE)")
    ...
    def post(self, args: RelaysStatusQuerySchema):
        ...
        logger.info(f"POST relays/  (REMOTE)")
...
class WifiBandsStatusApi(MethodView):
    ...
    def get(self, relay: str):
        ...
        logger.info(f"GET relays/sinle/{relay}  (REMOTE)")
